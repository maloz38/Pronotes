...
export default function App() {
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      // In production, if served from the same origin or proxied, a relative path works best.
      // If developing locally, we might need the full URL if ports differ.
      // We try to use import.meta.env.VITE_API_URL if available, or fallback to relative path /api/notes
      // However, since we are running separate servers (3001 for backend, 3000/5173 for frontend dev),
      // we default to http://localhost:3001 for dev if not specified.
